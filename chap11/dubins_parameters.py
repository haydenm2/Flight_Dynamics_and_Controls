...
class dubins_parameters:
    ...
    def update(self, ps, chis, pe, chie, R): # see supplement pg. 204
        ell = np.linalg.norm(ps - pe)
        if ell < 2 * R:
            print('Error in Dubins Parameters: The distance between nodes must be larger than 2R.')
        else:
            crs = ps + R * rotz(np.pi / 2.0) @ np.array([[np.cos(chis)], [np.sin(chis)], [0.0]])
            cls = ps + R * rotz(-np.pi / 2.0) @ np.array([[np.cos(chis)], [np.sin(chis)], [0.0]])
            cre = pe + R * rotz(np.pi / 2.0) @ np.array([[np.cos(chie)], [np.sin(chie)], [0.0]])
            cle = pe + R * rotz(-np.pi / 2.0) @ np.array([[np.cos(chie)], [np.sin(chie)], [0.0]])
            ...
            L4 = np.linalg.norm(cls - cle) + R*mod(2.0*np.pi + mod(chis + np.pi/2.0) - mod(theta + np.pi/2.0)) + R*mod(2.0*np.pi + mod(theta + np.pi/2.0) - mod(chie + np.pi/2.0))
            ...
            self.r3 = pe  # denoted as z3 in the supplement
            self.n3 = rotz(chie) @ e1  # denoted as q3 in the supplement
