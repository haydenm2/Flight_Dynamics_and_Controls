...
class mav_dynamics:
    ...
    def update_sensors(self):
        ...
        # ---------BAROMETER-----------
        beta_abs_press = 0.0  # temperature related bias drift
        beta_diff_press = 0.0  # temperature related bias drift
        self.sensors.static_pressure = MAV.rho*MAV.gravity*h + beta_abs_press + n_abs_press
        self.sensors.diff_pressure = MAV.rho*Va**2/2.0 + beta_diff_press + n_diff_press
        ...
        if self._t_gps >= SENSOR.ts_gps:
            ...
            self._gps_eta_e = np.exp(-k*Ts)*self._gps_eta_n + n_gps_e
            self._gps_eta_h = np.exp(-k*Ts)*self._gps_eta_n + n_gps_h
            self.sensors.gps_n = pn + self._gps_eta_n
            self.sensors.gps_e = pe + self._gps_eta_e
            self.sensors.gps_h = -pd + self._gps_eta_h
            Vn = Va*np.sin(psi) + we
            Ve = Va*np.cos(psi) + wn
            self.sensors.gps_Vg = np.sqrt(Vn**2 + Ve**2) + n_gps_Vg
            self.sensors.gps_course = np.arctan2(Vn, Ve) + n_gps_course
            self._t_gps = 0.
        else:
    ...
    def _forces_moments(self, delta):
        ...
        m_a = (0.5 * rho * Va**2 * S) * np.array([[b*(C_l_0 + C_l_b*beta + C_l_p*(b/(2*Va))*p + C_l_r*(b/(2*Va))*r + C_l_da*delta_a + C_l_dr*delta_r)],
                       ...
                       [b*(C_n_0 + C_n_b*beta + C_n_p*(b/(2*Va))*p + C_n_r*(b/(2*Va))*r + C_n_da*delta_a + C_n_dr*delta_r)]])
        m_total = m_a + m_p
